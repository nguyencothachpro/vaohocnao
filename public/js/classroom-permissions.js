(() => {
  ...
  function init(socket){
    ...
    if(C.isTeacher){
      const bar=document.createElement('div');bar.id='writePermissionBar';bar.className='write-permission-bar teacher';bar.innerHTML='<span>👨‍🏫 Giáo viên có quyền viết.</span><button id="revokeAllWrite" class="btn btn-sm btn-outline-danger">Thu hồi tất cả</button>';document.querySelector('.classroom-stage')?.appendChild(bar);
      ...
      if(qm){
        qm.innerHTML='<div class="qmode-card"><div class="small fw-bold mb-2">❓ Hỏi giáo viên</div><label class="small d-flex align-items-center gap-2"><input id="questionModeToggle" type="checkbox"> Cho học viên đặt câu hỏi</label><div class="small text-muted mt-1">Tắt khi đang giảng để không bị làm phiền.</div></div>';
        const toggle=document.getElementById('questionModeToggle');let enabled=true;toggle.checked=enabled;toggle.onchange=()=>{enabled=toggle.checked;socket.emit('classroom:question-mode',{enabled})};socket.emit('classroom:question-mode',{enabled});
      }
    }
    socket.on('classroom:question-mode',({enabled})=>{const box=document.getElementById('questionBox');if(box&&!C.isTeacher)box.classList.toggle('d-none',!enabled)});
    socket.on('classroom:question',p=>{if(!C.isTeacher)return;addChatMessage('❓ '+(p.user||'Học viên'),p.text||'');markUnread()});
    ...
    socket.on('classroom:chat',m=>{if(m.senderId===socket.id)return;markUnread()});
    setWritable(canWrite);
  }
  function addChatMessage(user,text){const box=document.getElementById('chatMessages');if(!box)return;const d=document.createElement('div');d.className='chat-msg';d.innerHTML='<b></b><span></span>';d.querySelector('b').textContent=user;d.querySelector('span').textContent=text;box.appendChild(d);box.scrollTop=box.scrollHeight}
  function markUnread(){const panel=document.getElementById('panel-chat'),badge=document.getElementById('chatBadge');if(!panel||!badge||panel.classList.contains('active'))return;badge.textContent=String((Number(badge.textContent)||0)+1);badge.classList.remove('d-none')}
