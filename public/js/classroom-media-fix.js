...
/* Make the existing classroom camera/mic code honor the selected devices and use a sensible HD capture profile. */
...
/* Add the selected TURN/STUN servers when supplied by the page. This wrapper runs before users create peers. */
...
async function applyOutput(el){if(!prefs.audioOutputId||typeof el.setSinkId!=='function')return;try{await el.setSinkId(prefs.audioOutputId)}catch(err){console.warn('setSinkId',err)} }
async function unlockAllAudio(){
  let played=0;
  for(const el of mediaEls()){
    if(el.srcObject){el.autoplay=true;el.playsInline=true;if(el.id!=='c3CameraPip')el.muted=false;await applyOutput(el);try{await el.play();played++}catch(_){} }
  }
...
function monitorMedia(){
  for(const el of mediaEls()){
    ...
    el.addEventListener('loadedmetadata',()=>{applyOutput(el);if(!el.muted)el.play?.().catch(()=>showUnlock())});
...
async function enumerate(){
  ...
  try{
    ...
    fill('c3MicSelect',devices.filter(d=>d.kind==='audioinput'),'Chọn micro');
    fill('c3CameraSelect',devices.filter(d=>d.kind==='videoinput'),'Chọn camera');
    fill('c3OutputSelect',devices.filter(d=>d.kind==='audiooutput'),'Âm thanh mặc định');
  }catch(err){console.error(err);toast('Không đọc được danh sách thiết bị')}
}
function fill(id,devices,defaultLabel){const s=$(id);if(!s)return;const old=s.value;s.innerHTML='';const first=document.createElement('option');first.value='';first.textContent=defaultLabel;s.appendChild(first);devices.forEach((d,i)=>{const o=document.createElement('option');o.value=d.deviceId;o.textContent=d.label||((d.kind==='audioinput'?'Micro ':d.kind==='videoinput'?'Camera ':'Thiết bị ')+(i+1));s.appendChild(o)});if([...s.options].some(o=>o.value===old))s.value=old;else if([...s.options].some(o=>o.value===(id==='c3MicSelect'?prefs.audioInputId:id==='c3CameraSelect'?prefs.videoInputId:prefs.audioOutputId)))s.value=id==='c3MicSelect'?prefs.audioInputId:id==='c3CameraSelect'?prefs.videoInputId:prefs.audioOutputId}

async function requestLabels(){
  try{const s=await navigator.mediaDevices.getUserMedia({audio:true,video:true});s.getTracks().forEach(t=>t.stop());await enumerate();toast('Đã tải danh sách micro/camera');}
  catch(err){console.error(err);await enumerate();toast('Hãy cấp quyền micro/camera để thấy tên thiết bị')}
}
...
function ensurePanel(){
  ...
  $('c3LoadDevices').onclick=requestLabels;$('c3SelectOutput').onclick=selectOutputPrompt;$('c3MediaTestMic').onclick=testMic;
  enumerate();
}
...
async function testMic(){
  try{
    const s=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true,channelCount:1,sampleRate:48000}});
    const ctx=new (window.AudioContext||window.webkitAudioContext)();const src=ctx.createMediaStreamSource(s);const a=ctx.createAnalyser();a.fftSize=256;src.connect(a);const data=new Uint8Array(a.fftSize);const bar=$('c3MediaLevelBar'),status=$('c3MediaStatus');status.textContent='Đang test micro… Hãy nói bình thường.';let until=Date.now()+4000;const tick=()=>{a.getByteTimeDomainData(data);let sum=0;for(const x of data){const n=(x-128)/128;sum+=n*n}const rms=Math.sqrt(sum/data.length);if(bar)bar.style.width=Math.min(100,Math.round(rms*320))+'%';if(Date.now()<until)requestAnimationFrame(tick);else{bar.style.width='0';status.textContent='Test mic xong — nếu thanh nhảy rõ, micro hoạt động tốt.';s.getTracks().forEach(t=>t.stop());ctx.close()}};tick();}catch(err){toast('Không test được micro: '+err.name)}
}
function observeConnections(){
  const Native=window.RTCPeerConnection;if(!Native)return;
  const observer=new MutationObserver(muts=>{monitorMedia();for(const m of muts)for(const n of m.addedNodes||[])if(n.nodeType===1&&n.matches?.('audio,video')){applyOutput(n);n.play?.().catch(showUnlock)}});observer.observe(document.body,{childList:true,subtree:true});
}
function boot(){if(!document.querySelector('.classroom-v3'))return;ensurePanel();injectButton();monitorMedia();observeConnections();document.addEventListener('pointerdown',()=>{unlockAllAudio().catch(()=>{})},{passive:true});navigator.mediaDevices?.addEventListener?.('devicechange',enumerate);setTimeout(monitorMedia,500);setTimeout(monitorMedia,1500)}
