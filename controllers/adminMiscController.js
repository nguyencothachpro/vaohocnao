...
exports.activationCodes = async (req, res) => {
  ...
  const courses = await Course.all(), books = await Book.all(), onlineBooks = await OnlineBook.all();
  const r = await require('../config/db').query(`
    SELECT a.*, COALESCE(c.title,b.title,ob.title) product_title
    FROM activation_codes a
    LEFT JOIN courses c ON c.id=a.course_id
    LEFT JOIN books b ON b.id=a.book_id
    LEFT JOIN online_books ob ON ob.id=a.online_book_id
    ORDER BY a.created_at DESC LIMIT 1000`);
  res.render('admin/activation-codes', { courses, books, onlineBooks, codes:r.rows, selectedCourseId:null });
};

exports.generateActivationCodes = async (req, res) => {
  const { product_type='course', course_id, book_id, online_book_id, quantity, expiry_days } = req.body;
  const qty = Math.min(parseInt(quantity) || 1, 200);
  let expires_at = null;
  if (expiry_days && parseInt(expiry_days) > 0) {
    const d = new Date(); d.setDate(d.getDate() + parseInt(expiry_days)); expires_at = d.toISOString();
  }
  for (let i=0;i<qty;i++) {
    await ActivationCode.create(genActivationCode(), product_type==='course'?course_id:null, expires_at, {
      product_type, book_id:product_type==='book'?book_id:null, online_book_id:product_type==='online_book'?online_book_id:null,
      duration_days: expiry_days ? parseInt(expiry_days) : null
    });
  }
  res.redirect('/admin/ma-kich-hoat');
};
