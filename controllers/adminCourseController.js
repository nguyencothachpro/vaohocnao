...
exports.contentPage = async (req, res) => {
  ...
  res.render('admin/courses/content', { parentType:'course', parent:course, course, chapters });
};

exports.bookContentPage = async (req, res) => {
  const Book = require('../models/Book');
  const book = await Book.findById(req.params.id);
  if (!book) return res.redirect('/admin/sach');
  const chapters = await require('../models/ContentTree').fullTree('book', book.id);
  res.render('admin/courses/content', { parentType:'book', parent:book, course:book, chapters });
};
