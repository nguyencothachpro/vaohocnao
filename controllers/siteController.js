...
exports.home = async (req, res) => {
  ...
  const [categories, courses, books, onlineBooks, banners, news] = await Promise.all([
    Category.tree(), Course.published(), Book.published(), OnlineBook.published(), Banner.active(), News.published()
  ]);
  res.render('index', {
    categories, courses:courses.slice(0,8), books:books.slice(0,4), onlineBooks:onlineBooks.slice(0,4),
    banners, news:news.slice(0,4)
  });
};
