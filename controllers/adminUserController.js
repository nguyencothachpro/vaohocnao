...
exports.extendEnrollment = async (req,res)=>{
  ...
  const current=await require('../config/db').query('SELECT expires_at FROM enrollments WHERE user_id=$1 AND course_id=$2',[req.params.id,req.body.course_id]);
  let base=current.rows[0]?.expires_at && new Date(current.rows[0].expires_at)>new Date()?new Date(current.rows[0].expires_at):new Date();
  base.setDate(base.getDate()+days);
  await Enrollment.extend(req.params.id,req.body.course_id,base.toISOString());
  res.redirect('/admin/hoc-vien/'+req.params.id);
};
