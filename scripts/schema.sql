...
-- Bang luu session cho connect-pg-simple
...
-- ==========================================================
-- KINGEDU V9: Unified learning content + classroom foundation
-- ==========================================================
ALTER TABLE chapters ADD COLUMN IF NOT EXISTS book_id INTEGER REFERENCES books(id) ON DELETE CASCADE;
ALTER TABLE chapters ALTER COLUMN course_id DROP NOT NULL;
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chapters_one_parent_chk') THEN
    ALTER TABLE chapters ADD CONSTRAINT chapters_one_parent_chk CHECK ((course_id IS NOT NULL AND book_id IS NULL) OR (course_id IS NULL AND book_id IS NOT NULL));
  END IF;
END $$;
...
ALTER TABLE lesson_files ADD COLUMN IF NOT EXISTS file_source TEXT NOT NULL DEFAULT 'upload';

CREATE TABLE IF NOT EXISTS book_enrollments (
  id SERIAL PRIMARY KEY,
  user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  book_id INTEGER NOT NULL REFERENCES books(id) ON DELETE CASCADE,
  enrolled_at TIMESTAMPTZ DEFAULT now(),
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'activation_codes_product_chk') THEN
    ALTER TABLE activation_codes ADD CONSTRAINT activation_codes_product_chk CHECK (product_type IN ('course','book'));
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS classroom_rooms (
  ...
  teacher_id INTEGER REFERENCES users(id) ON DELETE SET NULL,
  linked_course_id INTEGER REFERENCES courses(id) ON DELETE SET NULL,
  linked_book_id INTEGER REFERENCES books(id) ON DELETE SET NULL,
  started_at TIMESTAMPTZ,
  ended_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ DEFAULT now()
);

CREATE INDEX IF NOT EXISTS chapters_course_position_idx ON chapters(course_id, position);
CREATE INDEX IF NOT EXISTS chapters_book_position_idx ON chapters(book_id, position);
CREATE INDEX IF NOT EXISTS lessons_chapter_position_idx ON lessons(chapter_id, position);
